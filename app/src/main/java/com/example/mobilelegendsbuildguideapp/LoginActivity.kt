                ...
                if (task.isSuccessful) {
                    ...
                    users.child(userID).addListenerForSingleValueEvent(object : ValueEventListener {
                        ...
                        override fun onDataChange(snapshot: DataSnapshot) {
                            ...
                            //savedInstances.currentUser.username = snapshot.child("username").value.toString()
                            //Toast.makeText(baseContext, "Username: " + Common.currentUser.username, Toast.LENGTH_SHORT).show()
                            //savedInstances.currentUser.email_address = snapshot.child("email_address").value.toString()
                            //Common.currentUser.password = snapshot.child("password").value.toString()
                        }
                    })
                    updateUI(user)
                } else {
